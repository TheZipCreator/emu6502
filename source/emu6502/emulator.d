/**
  Emulates the 6502 architecture
*/
module emu6502.emulator;
...
class Emu6502 {
  ...
  bool throwExceptionOnBreak = false; /// Whether or not to throw an exception on the brk ($00) command being executed
  ...
  /// Gets a region of memory (for debug)
  ubyte[] region(int start, int end) {
    ubyte[] res;
    import std.array : appender;
    auto ap = appender(&res);
    for(int i = start; i < end; i++)
      res ~= load(i);
    return res;
  }

  private string regionString(int start, int end) {
    ubyte[] r = region(start, end);
    import std.array  : appender;
